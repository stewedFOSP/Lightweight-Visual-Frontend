...
class Model(BaseAVModel):
    def __init__(
        ...
        super(Model, self).__init__(sample_rate=sample_rate)
        ...
        self.l_encoder = LipEncoder()
        # self.l_conv = nn.Sequential(
        #     # CausalConv1d(in_channels=68 * 2, out_channels=num_freqs, kernel_size=1),
        #     # nn.Conv1d(in_channels=512, out_channels=num_freqs, kernel_size=1),
        #     # nn.Conv1d(68*2, num_freqs, 1),
        #     nn.Conv1d(vpre_channels, vin_channels, 1),
        #     # nn.ReLU()
        #     nn.SiLU()
        # ) 
        self.l_conv = nn.Conv1d(vpre_channels, vin_channels, 1)
        self.L_TCN = nn.ModuleList([])
        self.v_fuse = nn.Sequential(
            nn.Conv1d(vin_channels+64, vin_channels, 1),
            nn.SiLU(inplace=True)
        )
        for _ in range(5):
            self.L_TCN.append(VisualConv1D(V=vin_channels, H=vin_channels))
    ...
    # Forward pass
    def forward(self, input_wav, roi, lmk):
        # input shape: (B, T)
        was_one_d = False
        if input_wav.ndim == 1:
            was_one_d = True
        ...
        # print('roi', roi.shape, 'lmk', lmk.shape)
        lip_emb = self.l_encoder(roi) # [B, C, t]
        lip_emb = F.interpolate(lip_emb, size=lmk.shape[1], mode='linear') # [B, C, T]

        lmk = lmk.flatten(2) # [B, T, 84*2]
        lmk = self.l_conv(lmk.transpose(1, 2)) # [B, F, T]
        for ii in range(5):
            lmk = self.L_TCN[ii](lmk) # [B, F, T]
        # lmk = F.interpolate(lmk, size=T, mode='linear') # [B, E, T]
        v = self.v_fuse(torch.cat([lip_emb, lmk], dim=1)) # [B, F, T]
        # v = self.video(v)
